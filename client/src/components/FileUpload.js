...
import axios from "axios";//to get interact with pinata
...
const FileUpload = ({ contract, account, provider }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (file) { 
      try {
        ...
        //scripts from pinata // helps to store image in ipfs netwrok which we upload using pinata as a service
        const resFile = await axios({
          method: "post",
          url: "https://api.pinata.cloud/pinning/pinFileToIPFS",
          data: formData,
          headers: {
            pinata_api_key: `f874545e34316846c0be`,
            pinata_secret_api_key: `b43c8e794152e8b987f87c97992542f74146daabc71cbff51e16cea0f5b409fe`,
  ...
  //helps to fetch images data
  const retrieveFile = (e) => {
    const data = e.target.files[0]; //files array of files object
    // console.log(data);
    const reader = new window.FileReader();//helps to read data
